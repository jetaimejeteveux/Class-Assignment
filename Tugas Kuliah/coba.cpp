...
void bubbleSort()
{
				...
				for(int i = 0; i<jmlCustomer-1; i++) {
					for(int j = 0; j<jmlCustomer-1-i; j++) {
...
void selectionSort()
{
				for(int current=0; current<jmlCustomer; current++)
				{
				    for(int j=current+1; j<jmlCustomer; j++)
					{
...
{
				int x, y, z;
			    for(z = jmlCustomer/2; z > 0; z /= 2)
			   	{
			      	for(y = z ; y < jmlCustomer; y++)
			      	{
			         	for(x = y-z; x >= 0; x -= z)
			         	{
